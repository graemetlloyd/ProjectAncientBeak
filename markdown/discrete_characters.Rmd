...
```{r install required CRAN packages, message=FALSE, warning=FALSE}
...
```{r install required GitHub packages, message=FALSE, warning=FALSE}
...
```{r make taxon groups with ancestors}
...
```{r perform adonis test with ancestors included}
...
```{r prune matrix by skeletal region}
...
```{r ordinate character subregions}
...
```{r subset regional data by time bin}
...
```{r subset regional data by group}
...
Brougham and Campione REF showed that body size can strongly influence discrete character morphospaces in dinosaurs and we can use the same technique to explore whether a similar effect is seen in birds.

First we need to import body mass estimates for our taxa (NB: these are not available for all species).

```{r import body mass data}
...
```{r ordinate data for mass analyses}
...
```{r perform series of mass tests}
...
We can then compile these results in the Campione and Broughma style [REF].

```{r compile mass results}
all_results <- c()
...
```{r calculate phylogenetic distances and ordination of those distances}
...
```{r phylogenetic mean pariwise distances by group}
...
```{r phylogenetic mean pairwise distances by time bin}
...
```{r build phylogenetic and morphological dispRity objects}
...
```{r bootstrap phylogenetic and morphological dispRity objects}
...
```{r calculate phylogenetic and morphological disparity metrics}
...
DROP CROWN TAXA
DO REGULAR TIME SERIES AND GROUP DISPARITIES
TRY TO CORRELATE MORPH AND PHYLO IF POSSIBLE?
